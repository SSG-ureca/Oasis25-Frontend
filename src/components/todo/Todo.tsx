...
export const Todo = () => {
  ...
  const addTodo = () => {
    ...
    setTodos((prev) => [
      ...prev,
      { id: Date.now(), time: newTime, text: newText, done: false },
    ]);
    setNewText("");
    setNewTime("--:--");
  ...
  return (
    <RestrictedArea
      ...
      <div
        ...
        <h2 className="text-base font-extrabold text-text tracking-wider">
          TODO LIST
        </h2>
      </div>
        ...
        <div
          ...
          <div className="relative flex flex-col gap-2">
            {sortedTodos.map((todo, index) => {
              ...
              const isFirstTimeGroup =
                index === 0 || sortedTodos[index - 1].time !== todo.time;
              ...
              return (
                <div
                  ...
                  <Panel
                    ...
                    {isEditingThis && editData ? (
                      ...
                      <div
                        ...
                        className={`min-w-0 flex-1 cursor-pointer truncate text-[13px] font-normal select-none transition-colors ${
                          todo.done
                            ? "text-text-muted line-through opacity-40"
                            : "text-text"
                        }`}>
                        {todo.text}
                      </div>
                    ...
                    <div
                      className={`shrink-0 flex items-center gap-2 transition-opacity ${isEditingThis ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
                      {isEditingThis && editData ? (
                        <button
                          type="button"
                          onClick={() => {
      ...
      {isGuest && (
        <div className="min-h-0 flex-1 flex items-center justify-center text-sm font-medium text-text-muted">
          로그인 후 이용할 수 있습니다.
        </div>
      )}
      </div>
