...
export default function PomodoroTimer({
    ...
    async function handleSaveEdit() {
        ...
        if (
            selectedPresetName === presets.find((p) => p.id === editingId)?.name
        ) {
                        ...
                        {isLoggedIn ? (
                            canAddPreset ? (
                                ...
                                <span className="block text-xs text-text-muted mt-3 pt-3 border-t border-gray-100">
                                    커스텀 프리셋은 최대 {MAX_CUSTOM_PRESETS}
                                    개까지 저장할 수 있습니다.
                                </span>
                            )
                        ) : (
                ...
                createPortal(
                    ...
                    >
                        <p className="text-(--color-text) font-semibold">
                            오늘의 총 집중시간 :{" "}
                            {formatTime(summary.focusSeconds * 1000)}
                        </p>
